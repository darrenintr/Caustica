...
def test_material_sky_bindings_after_split_buffers() -> None:
    """GUIDE_COUNT=24 uses bindings 3..26; material/sky samplers must follow without collision."""
    ...
    assert "binding = 27" in rchit and "blockSpecAtlas" in rchit
    assert "binding = 28" in rchit and "blockNormalAtlas" in rchit
    assert "binding = 29" in rmiss and "celestialsAtlas" in rmiss
    composite = (ROOT / "src/main/java/dev/comfyfluffy/caustica/rt/RtComposite.java").read_text(
        encoding="utf-8"
    )
    assert "GUIDE_COUNT = 24" in composite
